...
### Observe nearby. Coordinate anywhere. Bring evidence home.

[![Hardware](https://img.shields.io/badge/hardware-Cardputer_ADV-0e222e?style=flat-square)](#build)
...
**The portable Reconclave console for M5Stack Cardputer ADV.**
...
FieldDeck is a standalone field toolkit and a cooperating Reconclave node. It
can observe, inspect, and retain evidence without a coordinator; when joined to
a trusted Reconclave fleet it advertises its capabilities, accepts authorised
work, and contributes observations from its own network vantage.

> Use FieldDeck only on systems, radio environments, and networks you own or
> are explicitly authorised to assess.

## Relationship to Reconclave

- **Standalone:** local UI, field workflows, storage, diagnostics, and evidence
  remain useful without Reconclave Command.
- **Collective:** discovery, capability announcements, authenticated requests,
  coordinator leases, and shared evidence conventions connect it to the fleet.
- **Shared contract:** `shared/protocol/` and `shared/identity/` are vendored
  snapshots of [Reconclave](https://github.com/Zetascrub/Reconclave), which is
  authoritative for protocol and style-guide definitions.
- **Security boundary:** discovery is not trust. Protected work requires
  provisioned peer keys and a valid, operator-authorised scope.
...
workspace, then copy the FieldDeck header into this repository:
